...
const UpdateProject = (props) =>
{
    ...
    if(deadLine.deadline !== null)
    {
        ...
        if(form.name==="" || form.name===undefined)
        {
            ...
            await fetch(`http://localhost:8080/projects/${props.project.id}`,{
                method: 'PUT',
                headers: {
            ...
            const resp = await fetch(`http://localhost:8080/dates/project/${props.project.id}`,{
                    method: 'POST',
                    headers: {
                        'Authorization' : `Bearer ${localStorage.getItem("token")}`,
                        'Content-Type': 'application/json'
        ...
        console.log(form)
        await fetch(`http://localhost:8080/projects/${form.id}`,{
            method: 'PUT',
            headers: {
                'Authorization' : `Bearer ${localStorage.getItem("token")}`,
                'Content-Type': 'application/json'
